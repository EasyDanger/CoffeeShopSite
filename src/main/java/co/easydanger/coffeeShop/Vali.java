...
public class Vali {
	...
	public static boolean itemIsKey(MenuItem mi, Map<MenuItem, Integer> map) {
		Set<MenuItem> keys = map.keySet();
		Iterator<MenuItem> iter = keys.iterator();
		MenuItem key;
		do {
			key = iter.next();
			if (key.checkName(mi)) {
		...
		} while (iter.hasNext() && !key.checkName(mi));
		return false;
	}
